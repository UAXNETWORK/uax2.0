...
add_genesis_funds() {
  local keyname="$1"
  evmd genesis add-genesis-account "$keyname" 1000000000000000000000abandwidth --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  evmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000abandwidth --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  # --------- Finalize genesis ---------
  evmd genesis gentx "$VAL_KEY" 1000000000000000000000abandwidth --gas-prices ${BASEFEE}abandwidth --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
...
evmd start "$TRACE" \
	--pruning default \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0abandwidth \
	--evm.min-tip=0 \
	--home "$CHAINDIR" \

...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address, cancelled chan struct{}, reset *txpoolResetRequest) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		recheckDrops, _ := list.FilterSorted(func(tx *types.Transaction) bool {
			...
			dropErr := tolerateNonceGapErr(err)
			if dropErr != nil {
				// uax: TEMPORARY DEBUG LOG - surfaces the real reason a
				// queued tx gets silently dropped during recheck/promotion.
				log.Error("uax: dropping queued tx during recheck", "hash", tx.Hash(), "addr", addr, "nonce", tx.Nonce(), "err", dropErr)
			}
			return dropErr != nil
		})
